...
export function handlePlayerReady(socket: WebSocket) {
    ...
    if (room.players.length === 2) {
        const allReady = room.players.every((player: any) => player.ready === true);
...
export function handlePlayerNotReady(socket: WebSocket) {
    const roomId = (socket as any).roomId;
