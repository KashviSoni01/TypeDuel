...
export function handlePlayerFinished(socket: WebSocket, message: any) {
  ...
  const player = room.players.find((p: any) => p.socket === socket);
  ...
  const opponent = room.players.find((p: any) => p.socket !== socket);
